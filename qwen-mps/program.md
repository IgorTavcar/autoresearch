...
- Switch to a different base model. The base model is Qwen3.5-0.8B — do not change it.
- Use AdamW or any optimizer that stores full momentum buffers per parameter. This is a 16GB MPS machine — AdamW requires ~4GB extra for optimizer states and will OOM during eval. Adafactor, SGD, or other memory-efficient optimizers only.
- Change EVAL_BATCH_SIZE or the eval section at the end of train.py. Eval memory is tightly tuned for 16GB — increasing the eval batch size will OOM.
