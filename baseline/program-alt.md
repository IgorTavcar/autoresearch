...
6. **Initialize experiments.json**: Create `experiments.json` as an empty array `[]`. This is the structured experiment memory (see Experiment Log below).
...
- Maintain `experiments.json` and optionally `rescue_pool.json` — structured experiment memory files.
...
## Choosing What to Experiment On Next (UCB1 Dimension Selector)

Before every experiment, read `experiments.json`. Compute UCB1 for each of the seven dimensions:
...
Where:
- `mean_improvement(dim)` = average improvement across all experiments in this dimension (0.0 if none)
- `N` = total experiments logged
- `n(dim)` = experiments in this dimension (use 0.5 if none, to avoid divide-by-zero)
...
This balances exploration (trying under-explored dimensions) with exploitation (doubling down on dimensions that showed improvement). Untried dimensions always get a large exploration bonus, ensuring no dimension is permanently abandoned.

## Early Abort Gate

At the 90-second training checkpoint, compare current val loss to the baseline val loss at the same step from the last kept experiment.

If current val loss > baseline * 1.05, abort.
Log the run as discarded with `"early_abort": true` in `experiments.json`.
...
When discarding, note whether any sub-mechanism showed local promise before the regression (e.g., faster early convergence even though final val_bpb regressed).
...
Before any new experiment, scan `rescue_pool.json` for recombination candidates in the same dimension as the one selected by UCB1.
...
7. short text description of what this experiment tried (include the dimension label, e.g. "[optimizer] switched to Lion")
...
2	b2c3d4e	0.993200	0.993200	44.2	keep	[optimizer] increase LR to 0.04	2026-03-09T22:20:45
3	c3d4e5f	1.005000	0.993200	44.0	discard	[attention] switch to windowed attention	2026-03-09T22:26:12
4	d4e5f6g	0.000000	0.993200	0.0	crash	[architecture] double model width (OOM)	2026-03-09T22:31:58
```
...
- optimizer: N trials, best improvement X.XXX, UCB1 trend ...
- architecture: N trials, ...
(one line per dimension — derived from experiments.json)
...
3. **Consult UCB1 scores.** The dimension with the highest UCB1 exploration bonus is likely the least-explored — prioritize structural changes in that dimension.
4. **Check rescue_pool.json.** Look for promising sub-mechanisms from discarded experiments that could be recombined.
5. **Try structural changes**: fundamentally different architectures, not incremental tweaks. Examples:
...
6. **Rewind if needed.** If structural exploration leads to 3+ consecutive crashes or regressions, rewind to the last known good commit and try a *different* structural direction. Do not keep hammering the same failing approach.
...
1. **Plan** (UCB1-guided step):
   - Read `guidance.md` if it exists — follow any human directions.
   - Read `findings.md` if it exists — review what's known.
   - Read `experiments.json` — compute UCB1 scores for all 7 dimensions.
   - Print UCB1 scores. Select the highest-scoring dimension.
   - Check `rescue_pool.json` for recombination candidates in the selected dimension.
   - Check for stagnation (see above).
   - Propose a targeted change within the selected dimension. Write a one-line rationale.
2. Tune `train.py` with the chosen experimental idea by directly hacking the code.
3. git commit
4. Run the experiment: `uv run train.py > run.log 2>&1` (redirect everything — do NOT use tee or let output flood your context)
5. **Early abort check**: At the 90-second mark, if intermediate val loss > baseline * 1.05, abort the run. Log as discarded with `"early_abort": true`. Skip to step 10.
6. Read out the results: `grep "^val_bpb:\|^peak_vram_mb:" run.log`
7. If the grep output is empty, the run crashed. Run `tail -n 50 run.log` to read the Python stack trace and attempt a fix. If you can't get things to work after more than a few attempts, give up.
8. Record the results in the tsv (NOTE: do not commit the results.tsv file, leave it untracked by git)
9. Append a record to `experiments.json` with dimension, delta, val_bpb, baseline_bpb, improvement, status, and early_abort fields.
10. If val_bpb improved (lower), you "advance" the branch, keeping the git commit. If discarding, check for rescue pool candidates (partial promising mechanisms worth saving).
11. If val_bpb is equal or worse, you git reset back to where you started.
12. Every 5 experiments: update `findings.md` (include per-dimension summary from experiments.json) and commit it.
...
**NEVER STOP**: Once the experiment loop has begun (after the initial setup), do NOT pause to ask the human if you should continue. Do NOT ask "should I keep going?" or "is this a good stopping point?". The human might be asleep, or gone from a computer and expects you to continue working *indefinitely* until you are manually stopped. You are autonomous. If you run out of ideas, consult your UCB1 scores — the exploration bonus will always point to under-explored dimensions. The loop runs until the human interrupts you, period.

As an example use case, a user might leave you running while they sleep. With early abort, you can run ~16-18 experiments/hour instead of ~12, for a total of about 130-150 over the duration of the average human sleep. The user then wakes up to experimental results plus a structured `experiments.json` showing which dimensions drove improvement — a research artifact in its own right.
